...
import mjson.Json;
import templates.Activity;
import templates.ActivityLogRelationship;
import templates.City;
import templates.Person;
import templates.Place;
import templates.PlaceCityRelationship;



public class Migrator {
	
	static String KEYSPACE = "cities";
	...
	static void loadDataIntoGrakn(Input input, Grakn.Session session) throws IOException, URISyntaxException {
	  ...
	  static Collection<Input> initialiseInputs() {
		  ...
		  inputs.add(Activity.template());
		  inputs.add(City.template()); 
		  inputs.add(Place.template());
		  
		  inputs.add(PlaceCityRelationship.template());
		  inputs.add(ActivityLogRelationship.template());
